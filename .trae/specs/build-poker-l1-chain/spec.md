...
> **SPEC FROZEN** — 本 spec 已于 2026-06-27 冻结。累计修复 55 项安全问题（第一轮 30 项 SEC + 第二轮 25 项 SEC2）+ 4 项高风险实现规范（IMPL-SEC-1~4）。冻结后不再新增修复，新发现记入 v2 backlog。Phase 5 已拆分为 5a/5b/5c 三个子 Phase 并行推进。
...
#### Scenario: Sparse Merkle Tree 实现规范（IMPL-SEC-3 修复）

* **WHEN** 节点维护全局对象状态根（state root）或交易 Merkle 根（tx_merkle_root）

* **THEN** **IMPL-SEC-3 修复 — Sparse Merkle Tree 实现规范**：(1) 哈希函数统一 `blake2b_256`（与地址派生一致）；(2) domain separation：叶子 `H(0x00 || key || value)`，内部 `H(0x01 || left || right)`，默认空叶 `H(0x00 || key || b"")`；(3) state tree depth = 256（keyed by `blake2b_256(ObjectID)`），tx_merkle_tree depth = 256（keyed by tx_hash）；(4) 强制 caching：实现须维护空子树缓存（每层默认哈希预计算），单次对象更新 O(log n) = 256 次哈希；(5) 空 leaf 值统一 = 空字节串 `b""`（与 SEC-L5 修复一致）
...
* **THEN** 节点从 tagged pubkey 提取 33B compressed pubkey，用 secp256k1 ECDSA recoverable 验证签名（含 v recovery id）；**NEW-L1 修复 — 强制 low-s（BIP-62）**：校验 `s <= n/2`（n 为曲线阶数），**`s > n/2` 返回 `InvalidSignatureLowS`（拒绝，不规范化转换 — R3-C2 修正：规范化会接受 high-s 变体，无法消除延展性；同一私钥对同一消息的 high-s 与 low-s 签名视为不同签名，延展性消除）**；应用于所有 secp256k1 签名路径（tx / vertex / receipt / operator\_ack / ACK）；**SEC-L2 修复 — low-s 检查时机明确**：low-s 校验必须在签名解析后、pubkey 恢复前执行（即解析 `r || s || v` 后立即校验 `s <= n/2`），失败立即返回 `InvalidSignatureLowS` 不进入 ECDSA 验证流程 — 避免 attacker 用 high-s 签名浪费 validator 完整 ECDSA 验证计算资源（DoS 缓解）；通过后接受 tx；**IMPL-SEC-1 修复 — secp256k1 常数时间实现规范**：(1) 共识路径 secp256k1 验证须使用 `libsecp256k1` 绑定 crate（`secp256k1` crate ≥ 0.28），禁用纯 Rust 实现于主网共识路径；(2) 全程常数时间：v recovery、pubkey 比对、low-s 比较均须常数时间实现；(3) tagged pubkey tag 字节解析须常数时间（防 timing 侧信道泄露 scheme 信息）；(4) 硬件钱包签名验证路径与软件路径一致（不得 bypass）**
