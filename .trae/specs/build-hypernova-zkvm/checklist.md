...
- [ ] `poker_l1` 通过 `pub use` re-export 引用迁移后的 `ZkShuffleCcsCircuit` — **延至 Phase 11**（新旧 `CcsCircuit` trait 签名不兼容，须 BREAKING 迁移）
- [ ] 既有单元测试引用路径已更新 — **延至 Phase 11**（`phase5a_integration.rs` + `task36_zk_verifier.rs` bench 须迁移）
