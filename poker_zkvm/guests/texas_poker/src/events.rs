//! Texas Poker 事件定义（Phase 3.5 移植）。
//!
//! 原 `poker_l1/src/vm/contracts/texas_poker/events.rs` 的 no_std 移植。
//!
//! # 移植变更
//!
//! - **删除 serde**：原 `#[derive(Serialize, Deserialize)]` 已删除，
//!   仅保留 `BorshSerialize, BorshDeserialize`（ZKVM guest 不依赖 serde）
//! - **ObjectID/Address**：原 `crate::object_model::ObjectID` / `crate::Address`，
//!   guest 端用 `super::types::{ObjectID, Address}`（= `[u8; 32]` / `[u8; 20]`）
//! - **测试 dummy_table_id**：原 `ObjectID::new([0xFF; 20], 0)`，
//!   guest 端用 `[0xFF; 32]`（与 types.rs 测试一致）
//!
//! 事件分类（与 Move 端一致）：
...
// 复用 types.rs 中的本地类型别名（ObjectID = [u8; 32], Address = [u8; 20]）
use super::types::{Address, ObjectID};
...
mod tests {
    ...
    fn test_all_variants_borsh_serializable() {
        ...
        // 覆盖所有 39 个变体，确保 derive(BorshSerialize, BorshDeserialize) 正确。
