...
// ===========================================================================
// Merkle Path AIR 列布局常量（60 列，v5.1 self-contained）
// ===========================================================================

/// col 0-7：LeafHash（当前层的起始 hash，8×31-bit M31 limbs）。
...
pub const MERKLE_AIR_COL_PREV_PARENT_HASH_BASE: usize = 8;
/// col 16-23：SiblingHash（Path 中该层的 sibling hash）。
pub const MERKLE_AIR_COL_SIBLING_HASH_BASE: usize = 16;
/// col 24-31：ParentHash（计算得到的 parent hash）。
pub const MERKLE_AIR_COL_PARENT_HASH_BASE: usize = 24;
/// col 32-39：ComputedRoot（累积计算得到的 root，最后一行）。
pub const MERKLE_AIR_COL_COMPUTED_ROOT_BASE: usize = 32;
/// col 40：IsLeftChild（0=left, 1=right）。
pub const MERKLE_AIR_COL_IS_LEFT_CHILD: usize = 40;
/// col 41：LayerIdx（该层索引，0=leaf layer, height-1=root layer）。
pub const MERKLE_AIR_COL_LAYER_IDX: usize = 41;
/// col 42：IsLastLayer（最后一层标记）。
pub const MERKLE_AIR_COL_IS_LAST_LAYER: usize = 42;
/// col 43：IsPadding（padding 标记）。
pub const MERKLE_AIR_COL_IS_PADDING: usize = 43;
/// col 44-51：PoseidonIntermediate1（Poseidon round 中间值 1）。
pub const MERKLE_AIR_COL_POSEIDON_INTERMEDIATE1_BASE: usize = 44;
/// col 52-59：PoseidonIntermediate2（Poseidon round 中间值 2）。
pub const MERKLE_AIR_COL_POSEIDON_INTERMEDIATE2_BASE: usize = 52;

/// Merkle Path AIR 总列数（v5.1）。
pub const MERKLE_AIR_NUM_COLUMNS: usize = 60;
...
impl FrameworkEval for MerklePathAir {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        for i in 0..8 {
            ...
            let left_i = is_left.clone() * sibling_i.clone() + (one.clone() - is_left.clone()) * leaf_i.clone();
            let right_i = is_left.clone() * leaf_i.clone() + (one.clone() - is_left.clone()) * sibling_i.clone();
        ...
        let is_first_layer = (one.clone() - layer_idx.clone()) * (one.clone() - layer_idx.clone() + one.clone());
...
mod tests {
    ...
    fn test_merkle_path_air_num_columns() {
        assert_eq!(MERKLE_AIR_NUM_COLUMNS, 60);
    }
    ...
    fn test_merkle_path_air_column_layout_no_overlap() {
        ...
        for i in 0..8 {
            assert!(all_cols.insert(MERKLE_AIR_COL_LEAF_HASH_BASE + i), "LeafHash col {} 重复", i);
        }
        for i in 0..8 {
            assert!(all_cols.insert(MERKLE_AIR_COL_PREV_PARENT_HASH_BASE + i), "PrevParentHash col {} 重复", i);
        }
        for i in 0..8 {
            assert!(all_cols.insert(MERKLE_AIR_COL_SIBLING_HASH_BASE + i), "SiblingHash col {} 重复", i);
        }
        for i in 0..8 {
            assert!(all_cols.insert(MERKLE_AIR_COL_PARENT_HASH_BASE + i), "ParentHash col {} 重复", i);
        }
        for i in 0..8 {
            assert!(all_cols.insert(MERKLE_AIR_COL_COMPUTED_ROOT_BASE + i), "ComputedRoot col {} 重复", i);
        }
        assert!(all_cols.insert(MERKLE_AIR_COL_IS_LEFT_CHILD), "IsLeftChild 重复");
        assert!(all_cols.insert(MERKLE_AIR_COL_LAYER_IDX), "LayerIdx 重复");
        assert!(all_cols.insert(MERKLE_AIR_COL_IS_LAST_LAYER), "IsLastLayer 重复");
        assert!(all_cols.insert(MERKLE_AIR_COL_IS_PADDING), "IsPadding 重复");
        for i in 0..8 {
            assert!(all_cols.insert(MERKLE_AIR_COL_POSEIDON_INTERMEDIATE1_BASE + i), "PoseidonIntermediate1 col {} 重复", i);
        }
        for i in 0..8 {
            assert!(all_cols.insert(MERKLE_AIR_COL_POSEIDON_INTERMEDIATE2_BASE + i), "PoseidonIntermediate2 col {} 重复", i);
        }
    ...
    fn test_merkle_path_air_v5_1_constraint_count() {
        ...
        // M5-M12: 8×3=24 条 Poseidon252 hash 验证
        // M35: 8 条 chain propagation
        // M36: 8 条 first layer prev_parent_hash=0
        // M37: 8 条 final root check
        // 总计: 3 + 1 + 24 + 8 + 8 + 8 = 52 条
        const M1_M3_COUNT: usize = 3;
        const M4_COUNT: usize = 1;
        const M5_M12_COUNT: usize = 24;
        const M35_COUNT: usize = 8;
        const M36_COUNT: usize = 8;
        const M37_COUNT: usize = 8;
        const V5_1_TOTAL: usize = M1_M3_COUNT + M4_COUNT + M5_M12_COUNT + M35_COUNT + M36_COUNT + M37_COUNT;
        assert_eq!(V5_1_TOTAL, 52);
    }

    #[test]
    fn test_merkle_path_air_hash_columns_size() {
        // 验证 hash 列都是 8 列（Poseidon252 输出 = 8×31-bit M31 limbs）
        assert_eq!(MERKLE_AIR_COL_PREV_PARENT_HASH_BASE - MERKLE_AIR_COL_LEAF_HASH_BASE, 8);
        assert_eq!(MERKLE_AIR_COL_SIBLING_HASH_BASE - MERKLE_AIR_COL_PREV_PARENT_HASH_BASE, 8);
        assert_eq!(MERKLE_AIR_COL_PARENT_HASH_BASE - MERKLE_AIR_COL_SIBLING_HASH_BASE, 8);
        assert_eq!(MERKLE_AIR_COL_COMPUTED_ROOT_BASE - MERKLE_AIR_COL_PARENT_HASH_BASE, 8);
    }
}
