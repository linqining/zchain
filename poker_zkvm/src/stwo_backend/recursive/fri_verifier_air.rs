...
impl FrameworkEval for FriVerifierAir {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let query_eval: Vec<E::F> = (0..4)
            ...
            .collect();
        let coeff: Vec<E::F> = (0..4).map(|i| col(FRI_AIR_COL_COEFF_BASE + i)).collect();
        let gating = col(FRI_AIR_COL_GATING);

        // ===== F7: Gating = (1 - IsFirstRow) * (1 - IsPadding) =====
        let gating_expected =
            (one.clone() - is_first_row.clone()) * (one.clone() - is_padding.clone());
        eval.add_constraint(gating.clone() - gating_expected);

        // ===== F8a: M[k] = pe_prev[j] * query_x[l] =====
        let m = |i: usize| -> E::F { col(FRI_AIR_COL_M_BASE + i - 1) };
        ...
        let product0 = m(1).clone() - m(2).clone() + two.clone() * m(3).clone()
            - two.clone() * m(4).clone()
            - m(5).clone()
            - m(6).clone();
        ...
        let product1 = m(7).clone() + m(8).clone() + m(3).clone() - m(4).clone()
            + two.clone() * m(5).clone()
            + two.clone() * m(6).clone();
        ...
        let chain_gating =
            (one.clone() - is_last_layer.clone()) * (one.clone() - is_padding.clone());
        for i in 0..8 {
...
mod tests {
    ...
    fn test_fri_verifier_air_column_layout_no_overlap() {
        ...
        for i in 0..4 {
            assert!(
                all_cols.insert(FRI_AIR_COL_QUERY_EVAL_BASE + i),
                "QueryEval col {} 重复",
                i
            );
        }
        for i in 0..4 {
            assert!(
                all_cols.insert(FRI_AIR_COL_QUERY_X_BASE + i),
                "QueryX col {} 重复",
                i
            );
        }
        for i in 0..4 {
            assert!(
                all_cols.insert(FRI_AIR_COL_PARTIAL_EVAL_PREV_BASE + i),
                "PartialEvalPrev col {} 重复",
                i
            );
        }
        for i in 0..4 {
            assert!(
                all_cols.insert(FRI_AIR_COL_PARTIAL_EVAL_BASE + i),
                "PartialEval col {} 重复",
                i
            );
        }
        for i in 0..4 {
            assert!(
                all_cols.insert(FRI_AIR_COL_COEFF_BASE + i),
                "Coeff col {} 重复",
                i
            );
        }
        ...
        for i in 0..8 {
            assert!(
                all_cols.insert(FRI_AIR_COL_LAYER_COMMITMENT_BASE + i),
                "LayerCommitment col {} 重复",
                i
            );
        }
        for i in 0..8 {
            assert!(
                all_cols.insert(FRI_AIR_COL_NEXT_LAYER_COMMITMENT_BASE + i),
                "NextLayerCommitment col {} 重复",
                i
            );
        }
        for i in 0..8 {
            assert!(
                all_cols.insert(FRI_AIR_COL_FOLDING_ALPHA_BASE + i),
                "FoldingAlpha col {} 重复",
                i
            );
        }
        ...
        assert!(
            all_cols.insert(FRI_AIR_COL_IS_FIRST_LAYER),
            "IsFirstLayer 重复"
        );
        assert!(
            all_cols.insert(FRI_AIR_COL_IS_LAST_LAYER),
            "IsLastLayer 重复"
        );
        assert!(
            all_cols.insert(FRI_AIR_COL_FOLDING_VALID),
            "FoldingValid 重复"
        );
    ...
    fn test_fri_verifier_air_v5_1_constraint_count() {
        ...
        const V5_1_TOTAL: usize = F1_F6_COUNT
            + F7_COUNT
            + F8A_COUNT
            + F8B_COUNT
            + F9_COUNT
