...
/// felt252 的无损 M31 limb 数量。
const FELT252_M31_LIMBS: usize = 9;

/// M31 模数 `2^31 - 1`。limb 必须严格小于该值，不能使用 radix `2^31`
/// 后再把值 `2^31 - 1` 交给 `BaseField::from_u32_unchecked`。
const M31_MODULUS: u32 = (1u32 << 31) - 1;

/// 将 FieldElement252（Poseidon252 hash 输出）无损转换为 9 个合法 M31 limbs。
///
/// # P05-R gap #3 修复（limb 范围）
/// 此前实现直接把 felt252 的大端字节切成 8 个 32-bit chunk 并用 `from_u32_unchecked`
/// 装入 M31——但 32-bit chunk 可达 `2^32 - 1`，超出 M31 的 `2^31 - 1` 上界，导致后续
/// AIR 约束求值时 `partial_reduce` 在 debug profile 触发 add-with-overflow panic，
/// 在 release profile 静默产生不合法 M31 值。
///
/// 使用 radix `p = 2^31 - 1` 的小端分解。每个余数都严格 `< p`，因此是合法
/// `BaseField` canonical value；`p^9 > 2^252`，9 limbs 可覆盖完整 felt252。
fn field_element_252_to_m31_limbs(hash: &FieldElement252) -> [BaseField; FELT252_M31_LIMBS] {
    let mut quotient = hash.to_bytes_be();
    let mut limbs = [BaseField::zero(); FELT252_M31_LIMBS];

    for limb in &mut limbs {
        let remainder = div_rem_be_bytes(&mut quotient, M31_MODULUS);
        debug_assert!(remainder < M31_MODULUS);
        *limb = BaseField::from(remainder);
    }

    assert!(
        quotient.iter().all(|byte| *byte == 0),
        "9 base-M31 limbs must encode every felt252"
    );
    limbs
}

/// 对 256-bit big-endian 整数原地除以 `divisor`，返回余数。
fn div_rem_be_bytes(bytes: &mut [u8; 32], divisor: u32) -> u32 {
    let mut remainder = 0u64;
    for byte in bytes {
        let value = (remainder << 8) | u64::from(*byte);
        *byte = u8::try_from(value / u64::from(divisor))
            .expect("base-256 long-division quotient digit fits u8");
        remainder = value % u64::from(divisor);
    }
    u32::try_from(remainder).expect("remainder is smaller than the u32 divisor")
}
...
///
/// # 参数
/// - `cols` — Merkle trace 列（67 列）
/// - `target_log_size` — 目标 log_size
///
/// # 返回
/// 新的 `Vec<Vec<BaseField>>`，每列长度为 `2^target_log_size`
#[must_use]
pub(crate) fn pad_merkle_trace_to_log_size(
    cols: Vec<Vec<BaseField>>,
    target_log_size: u32,
) -> Vec<Vec<BaseField>> {
...
pub fn extract_fri_query_from_l1(
    ...
    let mut fri_verifier =
        FriVerifier::<Poseidon252MerkleChannel>::commit(&mut channel, fri_config, fri_proof, bound)
            .ok()?;
...
/// P05-R gap #1：从 L1 proof 的 Fiat-Shamir transcript 重新采样 FRI query positions。
///
/// 这是 [`extract_fri_query_from_l1`] 的伴生函数：后者在内部采样了 query positions
/// 但只返回折叠后的 `(query_x, query_eval)`，丢弃了 first-layer positions。本函数
/// 镜像同一 channel 重放流程，返回原始 first-layer `query_positions`，供调用方填入
/// `RecursivePublicInputs::query_positions`，使 Merkle Path AIR 不再走空-query 的
/// no-op 分支（见 `ensure_nonempty_public_inputs`）。
///
/// # 算法
/// 与 [`extract_fri_query_from_l1`] 完全一致地重放 L1 verifier 的 commit/POW/sample
/// 序列，区别仅在于第 8 步直接返回 `sample_query_positions` 的结果而非折叠它。
///
/// # 参数
/// - `l1_proof` — L1 Stwo proof
/// - `config` — L1 proof 的 PcsConfig（须与 L1 verifier 一致）
/// - `max_log_degree_bound` — composition polynomial 的 max log degree bound
/// - `last_layer_poly` — L1 FRI last layer polynomial（仅用于驱动 FriVerifier 构造）
///
/// # 返回
/// - `Some(query_positions)` — 重放成功，返回非空 first-layer positions
/// - `None` — L1 proof 结构不匹配或 FriVerifier 构造失败
///
/// # 限制
/// 同 [`extract_fri_query_from_l1`]：仅支持单组件 proof（无 interaction phase）。
#[allow(clippy::missing_errors_doc)]
pub fn extract_query_positions_from_l1(
    l1_proof: &StarkProof<Poseidon252MerkleHasher>,
    config: PcsConfig,
    max_log_degree_bound: u32,
    last_layer_poly: &LinePoly,
    ...
    let bound = CirclePolyDegreeBound::new(max_log_degree_bound);
    let fri_proof = l1_proof.0.fri_proof.clone();
    let _ = last_layer_poly; // 仅保持签名与 extract_fri_query_from_l1 对称；FriVerifier 不直接消费它。
    ...
    let query_positions = fri_verifier.sample_query_positions(&mut channel);
    (!query_positions.is_empty()).then_some(query_positions)
}
...
///
/// # P05-R gap #1 前置条件
/// 调用方（`prove_recursive` / `prove_recursive_with_fri`）已通过
/// `ensure_nonempty_public_inputs` 校验 `l1_commitments`/`query_positions`/`log_size`
/// 均非空。因此此处不再保留空-input 的 no-op 早退分支；若 L1 proof 的
/// `queried_values`/`decommitments` 缺失（即无真实 decommitment），下游
/// [`compute_leaf_hash`] / [`extract_sibling_hash`] 会 panic——这是 prover 内部不变式：
/// 真实 L1 proof 必带 decommitment，伪造或缺失即内部错误而非可降级路径。
#[allow(clippy::missing_errors_doc)]
pub(crate) fn gen_merkle_path_trace(
    l1_proof: &StarkProof<Poseidon252MerkleHasher>,
    public_inputs: &RecursivePublicInputs,
) -> Vec<Vec<BaseField>> {
    use super::merkle_path_air::{
        MERKLE_AIR_COL_COMPUTED_ROOT_BASE, MERKLE_AIR_COL_IS_LAST_LAYER,
        MERKLE_AIR_COL_IS_LEFT_CHILD, MERKLE_AIR_COL_IS_PADDING, MERKLE_AIR_COL_LAYER_IDX,
        MERKLE_AIR_COL_LEAF_HASH_BASE, MERKLE_AIR_COL_PARENT_HASH_BASE,
        MERKLE_AIR_COL_POSEIDON_INTERMEDIATE1_BASE, MERKLE_AIR_COL_POSEIDON_INTERMEDIATE2_BASE,
        MERKLE_AIR_COL_PREV_PARENT_HASH_BASE, MERKLE_AIR_COL_SIBLING_HASH_BASE,
        MERKLE_AIR_NUM_COLUMNS,
    };
    ...
    // num_queries == 0 的早退分支已删除（P05-R gap #1）：空 query_positions 会让
    // Merkle Path AIR 完全不参与 L2 proof，是不 sound 的 no-op。调用方入口已拒绝。
    debug_assert!(
        num_queries > 0,
        "ensure_nonempty_public_inputs should have rejected empty query_positions"
    );
    ...
    for (query_idx, &query_pos) in public_inputs.query_positions.iter().enumerate() {
        ...
        let mut prev_parent_hash = [BaseField::zero(); FELT252_M31_LIMBS];
        ...
        for layer_idx in 0..tree_height {
            ...
            for i in 0..FELT252_M31_LIMBS {
                ...
                let left_i = if is_left_child {
                    current_hash[i]
                } else {
                    sibling_hash[i]
                };
                let right_i = if is_left_child {
                    sibling_hash[i]
                } else {
                    current_hash[i]
                };
            ...
            cols[MERKLE_AIR_COL_IS_LEFT_CHILD][row] =
                BaseField::from_u32_unchecked(is_left_child as u32);
...
///
/// # PoC 限制（P05-R gap #3，未闭合）
/// 当 `queried_values` 为空或某列为空时，本函数回退到**确定性的占位 hash**
/// （而非报错）。这只是让 PoC trace 在缺数据时仍可生成，**不构成 sound 的 leaf 绑定**：
/// 一个 leaf 为占位值的 Merkle path 仍可通过 AIR 的 hash-chain 约束。
/// `ensure_nonempty_public_inputs` 已保证 `query_positions` 非空（即 Merkle Path AIR
/// 一定参与 L2 proof），但「leaf/sibling 真实绑定到 L1 proof 的具体 decommitment」
/// 仍需密码学专家完成，属 P05-R 已知未闭合项。
fn compute_leaf_hash(
    l1_proof: &StarkProof<Poseidon252MerkleHasher>,
    _query_pos: usize,
) -> [BaseField; FELT252_M31_LIMBS] {
    let queried_values = &l1_proof.0.queried_values;
    if queried_values.is_empty() || queried_values[0].is_empty() {
        panic!("L1 proof is missing queried values for the Merkle opening");
    }
    ...
    assert!(!values.is_empty(), "L1 proof Merkle leaf cannot be empty");
...
///
/// # PoC 限制（P05-R gap #3，未闭合）
/// 当 `decommitments` 为空或 `hash_witness` 越界时，本函数回退到**全零占位 hash**。
/// 同 [`compute_leaf_hash`]，这是 PoC 可运行性回退，**不构成 sound 的 sibling 绑定**：
/// 一个全零 sibling 的 path 仍可满足 AIR 的 Poseidon hash-chain 约束。真实 sibling
/// 绑定到 L1 proof 的具体 `MerkleDecommitmentLifted` 仍需密码学专家完成。
fn extract_sibling_hash(
    l1_proof: &StarkProof<Poseidon252MerkleHasher>,
    query_idx: usize,
    layer_idx: usize,
    tree_height: usize,
    ...
    assert!(
        !decommitments.is_empty(),
        "L1 proof is missing Merkle decommitments"
    );

    let tree_decommitment = &decommitments[0];
    let witness_idx = query_idx * tree_height + layer_idx;

    let sibling = tree_decommitment
        .hash_witness
        .get(witness_idx)
        .expect("compressed Stwo witness cannot be indexed as a dense query-major path");
    field_element_252_to_m31_limbs(sibling)
}

/// 计算 parent hash = Poseidon252(left, right)。
fn compute_parent_hash(
    left: &[BaseField; FELT252_M31_LIMBS],
    right: &[BaseField; FELT252_M31_LIMBS],
    is_left_child: bool,
) -> [BaseField; FELT252_M31_LIMBS] {
...
/// 从 9 个 base-M31 limbs 无损重建 felt252。
fn construct_felt252_from_m31s(limbs: &[BaseField; FELT252_M31_LIMBS]) -> FieldElement252 {
    let mut bytes = [0u8; 32];
    for limb in limbs.iter().rev() {
        let mut carry = u64::from(limb.0);
        for byte in bytes.iter_mut().rev() {
            let value = u64::from(*byte) * u64::from(M31_MODULUS) + carry;
            *byte = value as u8;
            carry = value >> 8;
        }
        assert_eq!(carry, 0, "decoded felt252 must fit in 256 bits");
    }
    FieldElement252::from_bytes_be(&bytes)
...
mod tests {
    ...
    #[test]
    fn felt252_base_m31_encoding_is_lossless() {
        let values = [
            FieldElement252::ZERO,
            FieldElement252::ONE,
            FieldElement252::from(M31_MODULUS - 1),
            FieldElement252::from(M31_MODULUS),
            FieldElement252::MAX,
        ];

        for value in values {
            let limbs = field_element_252_to_m31_limbs(&value);
            assert!(limbs.iter().all(|limb| limb.0 < M31_MODULUS));
            assert_eq!(construct_felt252_from_m31s(&limbs), value);
        }
    }

    #[test]
    fn felt252_encoding_preserves_bits_above_248() {
        let zero_limbs = field_element_252_to_m31_limbs(&FieldElement252::ZERO);
        let mut high_bytes = [0u8; 32];
        high_bytes[0] = 1;
        let high_value = FieldElement252::from_bytes_be(&high_bytes)
            .expect("2^248 is a canonical felt252 value");
        let high_limbs = field_element_252_to_m31_limbs(&high_value);

        assert_ne!(
            high_limbs, zero_limbs,
            "high felt252 bits must not be truncated"
        );
        assert_eq!(construct_felt252_from_m31s(&high_limbs), high_value);
    }
    ...
    fn test_extract_composition_oods_eval_from_real_l1_proof() {
        ...
        assert!(extracted.is_some(), "提取应成功（sampled_values 结构匹配）");
    }
    ...
    fn test_gen_oods_check_trace_v5_1_from_real_l1_proof() {
        ...
        // 验证 IsPadding
        assert_eq!(
            trace_cols[OODS_AIR_COL_IS_PADDING][0],
            BaseField::from(0u32)
        );
        for row in 1..num_rows {
            assert_eq!(
    ...
    fn test_gen_oods_check_trace_v5_1_soundness_tampered_claimed() {
        ...
        let oods_point = CirclePoint::zero();
        let max_log_degree_bound = 10u32;
        let real_composition_oods_eval =
            extract_composition_oods_eval_from_l1(&l1_proof, oods_point, max_log_degree_bound)
                .expect("提取 composition_oods_eval 应成功");
        ...
        assert!(
            has_diff,
            "篡改 composition_oods_eval 后 Claimed 应不等于 Computed"
        );
    }
    ...
    fn test_gen_fri_verifier_trace_padding() {
        ...
        for row in (n_coeffs + 1)..num_rows {
            assert_eq!(
                trace_cols[FRI_AIR_COL_IS_PADDING][row], one,
                "padding row {} IsPadding 应为 1",
                row
            );
    ...
    fn test_gen_fri_verifier_trace_flags() {
        ...
        for row in 0..num_rows {
            ...
            assert_eq!(
                trace_cols[FRI_AIR_COL_IS_FIRST_ROW][row], expected,
        ...
        for row in 0..num_rows {
            let expected = if row == n_coeffs {
                one
            } else {
                BaseField::zero()
            };
            assert_eq!(
                trace_cols[FRI_AIR_COL_IS_LAST_ROW][row], expected,
    ...
    fn test_gen_fri_verifier_trace_gating() {
        ...
        for row in 0..num_rows {
            ...
            assert_eq!(
                trace_cols[FRI_AIR_COL_GATING][row], expected_gating,
                "Gating at row {} 不匹配",
                row
            );
        }
    ...
    fn test_gen_fri_verifier_trace_constant_poly() {
        ...
        assert_eq!(
            trace_cols[FRI_AIR_COL_IS_FIRST_ROW][0],
            BaseField::from(1u32)
        );
        for i in 0..4 {
            assert_eq!(
        ...
        assert_eq!(
            trace_cols[FRI_AIR_COL_IS_LAST_ROW][1],
            BaseField::from(1u32)
        );
    ...
    fn test_gen_fri_verifier_trace_from_real_l1_proof() {
        ...
        let n_coeffs = last_layer_poly.len();
        assert!(n_coeffs > 0, "last_layer_poly 应至少有 1 个系数");
        assert!(
    ...
    fn test_pad_oods_trace_to_log_size_pad_to_4() {
        ...
        for row in 4..16 {
            assert_eq!(
                padded[OODS_AIR_COL_IS_PADDING][row], one,
    ...
    fn test_pad_fri_trace_to_log_size_pad_to_larger() {
        ...
        for row in 4..16 {
            assert_eq!(
                padded[FRI_AIR_COL_IS_PADDING][row], one,
                "padding row {row} IsPadding 应为 1"
            );
        }
