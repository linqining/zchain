...
impl FrameworkEval for OodsCheckAir {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let le0_expected = sv(0, 0).clone() - sv(1, 1).clone() + two.clone() * sv(2, 2).clone()
            - sv(2, 3).clone()
            - sv(3, 2).clone()
        ...
        let le2_expected =
            sv(0, 2).clone() - sv(1, 3).clone() + sv(2, 0).clone() - sv(3, 1).clone();
        eval.add_constraint(left_eval(2) - le2_expected);

        // O9: LeftEval[3] = SV[0][3] + SV[1][2] + SV[2][1] + SV[3][0]
        let le3_expected =
            sv(0, 3).clone() + sv(1, 2).clone() + sv(2, 1).clone() + sv(3, 0).clone();
        ...
        let right_eval = |i: usize| -> E::F { col(OODS_AIR_COL_RIGHT_EVAL_BASE + i) };

        // O10: RightEval[0] = SV[4][0] - SV[5][1] + 2*SV[6][2] - SV[6][3] - SV[7][2] - 2*SV[7][3]
        let re0_expected = sv(4, 0).clone() - sv(5, 1).clone() + two.clone() * sv(6, 2).clone()
            - sv(6, 3).clone()
            - sv(7, 2).clone()
        ...
        let re2_expected =
            sv(4, 2).clone() - sv(5, 3).clone() + sv(6, 0).clone() - sv(7, 1).clone();
        eval.add_constraint(right_eval(2) - re2_expected);

        // O13: RightEval[3] = SV[4][3] + SV[5][2] + SV[6][1] + SV[7][0]
        let re3_expected =
            sv(4, 3).clone() + sv(5, 2).clone() + sv(6, 1).clone() + sv(7, 0).clone();
        ...
        // 所有约束 degree = 1（线性组合）
        let product = |i: usize| -> E::F { col(OODS_AIR_COL_PRODUCT_BASE + i) };

        // O30: Product[0] = m1 - m2 + 2*m3 - 2*m4 - m5 - m6
        let p0_expected = m(1).clone() - m(2).clone() + two.clone() * m(3).clone()
            - two.clone() * m(4).clone()
            - m(5).clone()
            - m(6).clone();
        ...
        let p1_expected = m(7).clone() + m(8).clone() + m(3).clone() - m(4).clone()
            + two.clone() * m(5).clone()
            + two.clone() * m(6).clone();
...
mod tests {
    ...
    fn test_oods_check_air_column_layout_no_overlap() {
        ...
        for i in 0..4 {
            assert!(
                all_cols.insert(OODS_AIR_COL_CLAIMED_BASE + i),
                "Claimed col {} 重复",
                i
            );
        }
        for i in 0..4 {
            assert!(
                all_cols.insert(OODS_AIR_COL_COMPUTED_BASE + i),
                "Computed col {} 重复",
                i
            );
        }
        ...
        for i in 0..4 {
            assert!(
                all_cols.insert(OODS_AIR_COL_DF_X_BASE + i),
                "DfX col {} 重复",
                i
            );
        }
        for i in 0..32 {
            assert!(
                all_cols.insert(OODS_AIR_COL_SV_BASE + i),
                "SV col {} 重复",
                i
            );
        }
        for i in 0..4 {
            assert!(
                all_cols.insert(OODS_AIR_COL_LEFT_EVAL_BASE + i),
                "LeftEval col {} 重复",
                i
            );
        }
        for i in 0..4 {
            assert!(
                all_cols.insert(OODS_AIR_COL_RIGHT_EVAL_BASE + i),
                "RightEval col {} 重复",
                i
            );
        }
        ...
        for i in 0..4 {
            assert!(
                all_cols.insert(OODS_AIR_COL_PRODUCT_BASE + i),
                "Product col {} 重复",
                i
            );
        }
    ...
    fn test_oods_check_air_v5_1_constraint_count() {
        ...
        const V5_1_TOTAL: usize = O1_COUNT
            + O2_O5_COUNT
            + O6_O9_COUNT
            + O10_O13_COUNT
            + O14_O29_COUNT
