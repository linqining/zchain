...
use poker_zkvm::prover::generate_test_proof;
...
fn test_soundness_multiplicity_forgery_detected() {
    ...
    let (proof, commits) =
        LogUpProof::create(table.clone(), witness.clone(), multiplicity.clone())
    ...
    let (forged_proof, forged_commits) =
        LogUpProof::create(table, witness, forged_multiplicity)
...
fn test_soundness_logup_tampered_commitment_detected() {
    ...
    let (proof, commits) =
        LogUpProof::create(table, witness, multiplicity).expect("创建应成功");
...
fn test_soundness_trace_tampering_detected() {
    ...
    let valid_witness = vec![Fr::one(), Fr::from_u32_with_wrap(5), Fr::from_u32_with_wrap(5)];
    assert!(
    ...
    let tampered_witness = vec![Fr::one(), Fr::from_u32_with_wrap(5), Fr::from_u32_with_wrap(6)];
...
fn test_soundness_tampered_proof_payload_fails() {
    let (mut proof_bytes, public_io) = generate_test_proof();
    // 定位 u_l 字段：跳过 6B header + 4B CCS_len + CCS_bytes，u_l 紧随其后（32B Fr）
    let ccs_len = u32::from_le_bytes([
        proof_bytes[6],
        proof_bytes[7],
        proof_bytes[8],
        proof_bytes[9],
    ]) as usize;
    let u_l_offset = 10 + ccs_len; // u_l 起始偏移
    assert!(
        u_l_offset + 32 <= proof_bytes.len(),
        "u_l 偏移 {u_l_offset} 超出 proof 长度 {}",
        proof_bytes.len()
    );
    // 翻转 u_l 的第一个字节（folded LCCCS 的 claimed sum，sumcheck 直接校验）
    proof_bytes[u_l_offset] ^= 0x01;
    let result = verify_production(&proof_bytes, &public_io);
    assert!(
        result.is_err(),
