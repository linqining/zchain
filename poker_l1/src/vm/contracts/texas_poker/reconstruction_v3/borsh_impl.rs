...
//! on the zchain default curve (`Bls12381Curve`, i.e. zgame `DefaultCurve`).
//!
...
//! `ElGamalCiphertextGeneric<Bls12381Curve>` / `ECPoint`.
//!
//! `ElGamalCiphertextGeneric<Bls12381Curve>` itself is implemented in zgame
...
use poker_protocol::crypto::curve::{Bls12381Curve, CurveScalar};
...
/// G1 compressed point byte length (BLS12-381 G1).
...
fn read_scalar<R: borsh::io::Read>(r: &mut R) -> borsh::io::Result<BlsScalar> {
    ...
    Ok(<BlsScalar as CurveScalar>::from_bytes_mod_order(&bytes))
}
...
impl BorshDeserialize for ReconstructionV3Statement<Bls12381Curve> {
    fn deserialize_reader<R: borsh::io::Read>(r: &mut R) -> borsh::io::Result<Self> {
        ...
        let user_readable_cards = (0..k)
            ...
            .collect::<Result<Vec<poker_protocol::crypto::curve::ElGamalCiphertextGeneric<Bls12381Curve>>, _>>()?;
...
mod tests {
    ...
    fn reconstruction_v3_borsh_rejects_unknown_version_and_huge_length() {
        assert!(borsh::from_slice::<ReconstructProofV3<Bls12381Curve>>(&[99]).is_err());

        let mut malicious = vec![RECONSTRUCTION_V3_PROOF_VERSION];
        malicious.extend_from_slice(&u32::MAX.to_le_bytes());
        assert!(borsh::from_slice::<ReconstructProofV3<Bls12381Curve>>(&malicious).is_err());
    }
}
