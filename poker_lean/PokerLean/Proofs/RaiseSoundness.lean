...
theorem raise_air_not_sound :
  ...
  · -- Part 2: ¬ ContractRaise ...
    intro h
    have hbetting :
      (extractPreTableFromRaiseAir row 2).round_state.is_betting_round := by
      rcases h with ⟨h1, _⟩
      exact h1
    have hrs :
      (extractPreTableFromRaiseAir row 2).round_state = RoundState.ROUND_WAITING := by rfl
    rw [hrs] at hbetting
