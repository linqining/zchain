...
/-- bet AIR 不是 sound 的：在 ROUND_WAITING 状态下 bet 的反例 -/
theorem bet_air_not_sound :
  ...
  let ext : BetMethodColumns := {
    input_seat_index := nat_to_m31 0 hlt0
    input_bet_amount := (M31.zero, M31.zero, M31.zero, M31.zero)
  ...
  let base_row : CommonRow := {
    is_active := M31.one
    method_kind := ⟨MethodKind.Bet.toNat, MethodKind.toNat_lt_M31P MethodKind.Bet⟩
  ...
  let pre_table := extractPreTableFromBetAir base_row 2
  let post_table := extractPostTableFromBetAir base_row ext 2 0
  ...
  · -- Part 1: BetAirAcceptable row ext 0 2 0 hlt0
    unfold BetAirAcceptable
    refine ⟨?_, ?_, ?_, ?_⟩
    · -- CommonConstraints
      ...
      · have hsub : M31.sub row.method_kind
          ⟨MethodKind.Bet.toNat, MethodKind.toNat_lt_M31P MethodKind.Bet⟩ = M31.zero := by
          simp [row]
          apply sub_self_eq_zero
        rw [hsub]
        apply M31.mul_zero_right
    · -- BetMethodConstraints
      unfold BetMethodConstraints
      intro h_active
      have h_seat : ext.input_seat_index = nat_to_m31 0 hlt0 := by simp [ext, nat_to_m31]
      have h_amt : ext.input_bet_amount.1 = ⟨0 % 65536, M31_P_pos⟩ := by simp [ext]; rfl
  ...
  · -- Part 2: ¬ ContractBet ...
    ...
    have h_seat_empty :
      (extractPreTableFromBetAir row 2).get_seat 0 = Seat.empty := by
        simp [extractPreTableFromBetAir, TexasPokerTable.get_seat, List.getD, List.replicate]
    have h_not_participating :
      ¬ ((extractPreTableFromBetAir row 2).get_seat 0 |>.is_participating) := by
        rw [h_seat_empty]
