...
theorem bet_air_not_sound :
  ...
  · -- Part 1: BetAirAcceptable
    ...
    · -- BetMethodConstraints
      ...
      have h_amt : ext.input_bet_amount.1 = ⟨0 % 65536, by unfold M31_P; omega⟩ := by simp [ext]; rfl
  ...
  · -- Part 2: ¬ ContractBet ...
    intro h
    have hbetting :
      (extractPreTableFromBetAir row 2).round_state.is_betting_round := by
      rcases h with ⟨h1, _⟩
      exact h1
    have hrs :
      (extractPreTableFromBetAir row 2).round_state = RoundState.ROUND_WAITING := by rfl
    rw [hrs] at hbetting
