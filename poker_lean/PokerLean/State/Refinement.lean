...
/-!
# Phase 6b: 模型↔代码精化论证（Refinement）

本文件建立 Lean 状态机模型与 Rust `state_machine.rs` 实现之间的精化关系，
闭合「模型正确」到「代码正确」的最后一环。
...
4. **状态转移精化**（§4）：Rust 座位级 `apply_call/apply_raise`（用 `checked_*`）
   在 panic-freedom 下产出与 Lean `Seat.apply_call/apply_raise` 完全相同的状态。
...
/-! ### §5.2 桌台级 `apply_call` 精化（摘要）

桌台级 `apply_call`（`state_machine.rs:1986-2045`）= 座位级 `apply_call` 作用到
第 i 个 seat + `version + 1`。由 §4 座位级精化 + Lean 桌台级 chip 守恒
（`apply_call_chip_conservation`，Phase 2）⟹ 桌台级 Rust `apply_call` 守恒。

此定理将 Lean `total_chips` 守恒直接迁移到 Rust：在 `inv_chip_bounds` 下，
Rust `process_call` 三处 `checked_*` 均 panic-freedom，故 Rust 不早退、
字段值 = Lean 字段值，Lean `total_chips` 守恒即 Rust `total_chips` 守恒。 -/
theorem apply_call_rust_chip_conservation_via_refinement (t : TexasPokerTable)
    ...
    -- Lean 守恒（= Rust 守恒，因 panic-freedom 下字段值相同）
...
/-! ### §5.3 `apply_fold` / `apply_check` 精化（无 `checked_*`，trivial）

`apply_fold` / `apply_check` 无算术运算（仅置 bool 标记），Rust 中无 `checked_*`，
故无 panic 风险。Lean 守恒直接迁移到 Rust。 -/
...
Lean 已在 `Theorems.lean` 证明 `end_without_showdown_chip_conservation`。
Rust panic-freedom 由 §2.4 + §2.5 保证。 -/
theorem end_without_showdown_rust_chip_conservation (t : TexasPokerTable)
    ...
    (h_bounds : inv_chip_bounds t) :
    -- Rust chip 守恒 = Lean chip 守恒（panic-freedom 已由 §2.4/§2.5 保证）
...
| `apply_call`             | `Seat.apply_call`      | §2.1 + §5.2   | §5.2      |
| `apply_raise`            | `Seat.apply_raise`     | §2.2          | §5（座位级）|
| `apply_fold`             | `Seat.apply_fold`      | 无 `checked_*`| §5.3      |
| `apply_check`            | `Seat.apply_check`     | 无 `checked_*`| §5.3      |
| `collect_bets_to_pot`    | （Phase 6 摘要）       | §2.3          | Lean 已证 |
| `end_without_showdown`   | `end_without_showdown` | §2.4 + §2.5   | §5.4      |
...
1. **`advance_turn` / `collect_bets_to_pot` 的完整精化**：这些函数的 Lean 模型
   分布在 Phase 2-6 各模块，本文件仅给出摘要级精化（panic-freedom 已覆盖）。
...
4. **Rust 源码本身**：未在 Lean 中嵌入（无 Rust frontend），精化论证基于手工镜像。
   镜像保真度由类型映射（§1）的逐字段对应 + `state_machine.rs` 行号引用保证。
-/
