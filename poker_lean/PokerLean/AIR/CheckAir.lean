...
def extractPostTableFromCheckAir
    ...
    (seat_index : Nat)
    : TexasPokerTable :=
  let pre := extractPreTableFromCheckAir row ext max_players
  let post := { pre with
  ...
  post.update_seat seat_index (fun s => { s with acted_this_round := true })
...
def CheckMethodConstraints
  ...
  PotUnchanged row ∧
  ButtonUnchanged row ∧
  let pre_table := extractPreTableFromCheckAir row ext max_players
