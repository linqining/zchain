...
def AddonAirAcceptable
  ...
  AddonMethodConstraints row ext expected_seat_index expected_amount hlt ∧
...
def RebuyAirAcceptable
  ...
  RebuyMethodConstraints row ext expected_seat_index expected_amount hlt ∧
...
def extractPostTableFromFundsAir
    ...
    (max_players : Nat)
    : TexasPokerTable :=
  let pre := extractPreTableFromFundsAir row max_players
  { pre with
