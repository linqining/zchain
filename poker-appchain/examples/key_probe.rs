...
fn main() {
    ...
    println!("pub_from_5e_seed: {}", hex::encode(key.public));
    ...
    println!("frame index={}", frame.frame.index);
    let h = frame.hash().unwrap();
    let ok5e = SequencerKey::verify(&key.public, &h, &frame.sig);
    println!("verify with 5e-key: {ok5e}");
}
