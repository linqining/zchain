...
/// hot-v29 `pre_state_root`/`post_state_root` + 元数据，按**固定顺序** mix 进 channel。
...
pub struct TexasPublicInputs {
    ...
    /// 调用前 ObjectDb hot-v29 state root（从 `pre_image` 解码后重算）。
    pub pre_state_root: StateRoot,
    /// 调用后 ObjectDb hot-v29 state root（从 `post_image` 解码后重算）。
...
impl TexasPublicInputs {
    ...
    /// 验证方从完整 resolved-v28 image 解码 table，再按 ObjectDb hot-v29 projection 重算 root。
    ///
