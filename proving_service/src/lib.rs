...
//! 快照，再交给 `poker_texas_air` 的 Orchestrator 生成并聚合 STARK proof。
//!
//! ## 架构
//!
//! ```text
...
//! - [`runner::HandRunner`]：按牌局阶段顺序驱动 dispatch → 快照 → prove，串联 state_root 链。
//! - [`server`]：axum HTTP 服务（`POST /hands/run`、`POST /dispatch`、`GET /plugins`）。
//!
//! ## 设计原则
//!
...
pub use plugin::{ContractPlugin, PluginError, PluginResult, PluginStats, DispatchOutcome};
